...
bool Net::TraverseAddressFromHost(const char* host, std::vector<std::string>& list) {
  ...
  if (res == 0) {
    for (auto iter = result; iter != nullptr; iter = iter->ai_next) {
      ...
      res = getnameinfo(iter->ai_addr, static_cast<socklen_t>(iter->ai_addrlen), address, sizeof(address), nullptr, 0,
      ...
      if (res == 0) {
        ...
        std::cerr << "mToolkit net error:" << gai_strerror(res) << std::endl;
...
bool Net::CloseSocket(int32_t socket) { return ::close(socket) == 0; }
...
bool Net::EnableNonBlock(int32_t socket) { return fcntl(socket, F_SETFL, fcntl(socket, F_GETFL, 0) | O_NONBLOCK) == 0; }
...
int32_t Net::GetSendBufferSize(int32_t sock) {
  ...
  return getsockopt(sock, SOL_SOCKET, SO_SNDBUF, reinterpret_cast<char*>(&optionValue), &optionLength) == 0 ? optionValue
                                                                                                            : -1;
}
...
int32_t Net::GetReceiveBufferSize(int32_t sock) {
  ...
  return getsockopt(sock, SOL_SOCKET, SO_RCVBUF, reinterpret_cast<char*>(&optionValue), &optionLength) == 0 ? optionValue
                                                                                                            : -1;
}
...
uint32_t Net::AsHostByte(const char* value) { return inet_network(value); }

std::string Net::AsString(uint32_t value) {
  return std::to_string((value >> 24) & 0xFF) + "." + std::to_string((value >> 16) & 0xFF) + "." +
         std::to_string((value >> 8) & 0xFF) + "." + std::to_string((value >> 0) & 0xFF);
}
