...
const char *HTTPClient::LastError() const { return curl_easy_strerror(_errorCode); }
...
void HTTPClient::SetParam(std::string param) { _session.params = std::move(param); }

void HTTPClient::SetSkip(bool skip) { _session.skip = skip; }

void HTTPClient::SetCAFile(std::string caFile) { _session.caFile = std::move(caFile); }

void HTTPClient::SetCookie(std::string cookie) { _session.cookie = std::move(cookie); }
...
void HTTPClient::SetUserAgent(std::string userAgent) { _session.userAgent = std::move(userAgent); }

void HTTPClient::SetConnectTimeout(std::time_t timeout) { _session.timeout = timeout; }

void HTTPClient::SetReceiveTimeout(std::time_t timeout) { _session.receiveTimeout = timeout; }

void HTTPClient::SetRequestOption(std::string requestOption) { _session.requestOption = std::move(requestOption); }

void HTTPClient::AddParam(const std::string &param) { _session.params += param; }

void HTTPClient::AddHeader(const char *header) { _session.headers = curl_slist_append(_session.headers, header); }
...
bool HTTPClient::GlobalInitialize() { return curl_global_init(CURL_GLOBAL_ALL) == CURLE_OK; }
...
// 获取请求头
size_t HTTPClient::AskHeaderFunction(CURL *, curl_infotype type, void *buffer, size_t size, void *stream) {
  auto *client = reinterpret_cast<HTTPClient *>(stream);
