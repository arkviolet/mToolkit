...
class Builder : public std::enable_shared_from_this<Builder> {
 ...
 public:
  void Run(const std::shared_ptr<Message> &message) {
    _handler_list.ForEach([&](const std::shared_ptr<Handler> &handler) { OnHandler(handler, message); });
  }
 ...
 protected:
  void OnHandler(const std::shared_ptr<Handler> &handler, const std::shared_ptr<Message> &message) { handler->Run(message); }
